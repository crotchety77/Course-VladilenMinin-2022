...
// 7.4 были ли автоматические поражения. Если были, то выведите сообщение через
// alert “Были автоматические поражения: да”, иначе “Были автоматические
// поражения: нет”.

// if (goals.some((goal) => goal === -1)){
...
// Читаемость кода страдает, но это просто практика
// (lose === true) ? console.log("Были автоматические поражения: да") : console.log("Были автоматические поражения: нет");
// можно ли через ветвленийёе вставить ${?}

// ChatGPT
console.log("Были автоматические поражения: " + (goals.includes(-1) ? "да" : "нет"));

// 7.5
// Среднее количество голов за матч. Выведите сообщение через alert “Среднее
// количество голов за матч равно numberOfGoals” (замените numberOfGoals на
// среднее количество голов за матч).

const countMatches = goals.filter(goal => goal > 0).length;
numberOfGoals = (sumGoals / countMatches).toFixed(2);
console.log(`Среднее количество голов за матч равно ${numberOfGoals}`);

// 7.6
//Отсортируйте голы в порядке возрастания и выведите все результаты через
//запятую в модальном окне alert. Массив goals не должен быть изменен.

let goals2 = [...goals].sort().join(", "); 
// goals2.sort();
console.log(goals2);
